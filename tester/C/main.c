...
int main() {
    ...
    while (1) {
        ...
        if (internal_query != NULL && external_query != NULL) {
            int decrypt_len = aes_decrypt(internal_query, sizeof(internal_query), keys->aes_key, keys->aes_iv);
            int decrypt_len2 = aes_decrypt(external_query, sizeof(external_query), keys->aes_key, keys->aes_iv);
        }
        ...
        if (internal_query != NULL) {
            int encrypt_len2=aes_encrypt(internal_query, keys->aes_key, keys->aes_iv);
